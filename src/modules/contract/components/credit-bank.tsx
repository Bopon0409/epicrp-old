...
export const CreditBank = observer(() => {
  const {
    state: { names, date, currentProperty, rate, currentSum },
    clear, submit, dateSelectOptions, propertiesOptions, setCurrentDate,
    setCurrentProp, setCurrentSum, creditSumBank, dailyPaymentBank
  } = store
  ...
  return (
    <div className='tablet'>
      ...
      <div className='tablet__text'>
        ...
        <input type='number' className='sum-input' value={currentSum.toString()}
          onChange={setCurrentSum} />
        которые используются заёмщиком в личных целях.
        <br />
        ...
        <Select name='tariff' options={dateSelectOptions}
          onSelect={setCurrentDate} />включительно.
        <br />
        ...
        <Select name='tariff' options={propertiesOptions}
          onSelect={setCurrentProp} />включительно.
        оценочной стоимостью в ${currentProperty?.price}.
        <br />
        1.5 Сумма возвратных средств по настоящему договору ${creditSumBank},
        сумма процента по настоящему договору {rate * 100}% суточных, сумма
        ежедневного платежа ${dailyPaymentBank}.
      </div>
