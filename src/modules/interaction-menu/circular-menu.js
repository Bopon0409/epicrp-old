...
;(function (global, factory) {
  ...
  function createAnchor (parent, data, index) {
    ...
    if (data.onHoverIn && data.onHoverOut) {
      on(a, 'mouseenter', () => data.onHoverIn(data))
      on(a, 'mouseleave', () => data.onHoverOut(data))
    }
  ...
  function createText (parent, data) {
    ...
    style(
      span,
      'margin-top',
      hasIcon(data.icon) ? withIconMarginTop : this._calc.textTop
    )
  ...
  function createHorizontal (parent, data, index) {
    ...
    if (this._config.horizontal)
      style(
        div,
        'transform',
        'rotate(' + this._calc.horizontalDeg(index) + 'deg)'
      )
  ...
  function createSubMenu (creator, menus, index) {
    ...
    var startDeg =
      this._calc.rotateDeg(index) - totalAngle / 2 + this._calc.centralDeg / 2
