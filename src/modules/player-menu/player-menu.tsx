...
export const PlayerMenu = observer(() => {
  ...
  useEffect(() => {
    const {
      ...
      adminMsgSend,
      setSettingsData,
      reportAdminConnected,
      adminCloseReport,
      setQuests,
    } = store;
    const { setDonat, setCaseContent, setWinnerIndex, setTransactionResult } =
      donatStore;
    ...
    em.addHandler("player-menu.donat.case-content", setCaseContent);
    em.addHandler("player-menu.donat.winner-index", setWinnerIndex);
    em.addHandler("player-menu.donat.check-money", setTransactionResult);
