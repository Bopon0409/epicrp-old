...
const SCROLL_TIME = 3 * 1000; // время самолго скролла
const TIME_FOR_WAIT_AFTER_OPEN = 2 * 1000; // время ожидания, после прокрутки
...
export const LuckyCase = observer(() => {
  ...
  useEffect(() => {
    if (buyCase) {
      ...
      setTimeout(() => {
        setIsMarginLeft(`${(winningIndex - 1) * STEP}px`);
        // перемещаем до нужной позиции
      }, 1000);
    }
    ...
    <div className="lucky-case">
      ...
      <div className="upper-block">
        ...
        <div className="donat_and_warehouse">
          <div className="donat__warehouse">Мои выигрыши</div>
              ...
              >
                {caseContent.map((item, id) => (
                  <div className="roulette-item" key={id}>
                    <div
                      className="roulette-item_bg"
                      style={BackgroundTier[item.tier]}
                    />
                    <div className="roulette-item__img" />
        ...
        <div className="items-list">
          {donatProducts[activeStoreProduct].content?.map((item, id) => (
            <div className="items-list__item" key={id}>
              <div className="donatItem-bg" style={BackgroundTier[item.tier]} />
              <div className="donatItem-img" />
              <span className="donatItem-name">{item.name}</span>
            </div>
          ))}
        </div>
      </div>
