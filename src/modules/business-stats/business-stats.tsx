...
export const BusinessStats = observer(() => {
  ...
  const MenuBlocks =
    store.state.stats?.businessName[0] === "АВТОСАЛОН"
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      em.removeHandler(
        "business-stats.products-irl-items",
        setProductsIrlItems
      );
      ...
      em.removeHandler(
        "business-stats.warehouse-clearData",
        warehouseClearData
      );
      em.removeHandler(
        "business-stats.stats-setBusinessStatus",
        setBusinessStatus
      );
      em.removeHandler("business-stats.change-player-money", changePlayerMoney);
      em.removeHandler(
        "business-stats.change-business-money",
        changeBusinessMoney
      );
    };
  }, []);
  ...
  const keyClick = (e: React.KeyboardEvent<HTMLDivElement>) => {
    switch (e.keyCode) {
      case 81:
        return makePreviousBlockActive();
      case 69:
        return makeNextBlockActive();
      default:
        return;
    }
  };
  ...
  return store.state.active ? (
    <div className="business-stats" onKeyUp={keyClick} tabIndex={0} ref={main}>
      ...
      <div className="menu">
        <div className="menu_key" onClick={() => makePreviousBlockActive()}>
          Q
        </div>
        <div className="menu_blocks-line">
          {MenuBlocks.map((block, i) => (
            <div className="block" key={i}>
              <div className="content" onClick={() => store.setActiveBlock(i)}>
                {block}
              </div>
              <div
                className={cn("block__line", {
        ...
        <div className="line" />
        <div className="menu_key" onClick={() => makeNextBlockActive()}>
          E
        </div>
      </div>
      {currentPage()}
